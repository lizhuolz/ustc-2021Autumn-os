...
void scroll_screen(void) {
    ...
    for (int y = 0; y <= VGA_SCREEN_HEIGHT - 2; y++)
        for (int x = 0; x < VGA_SCREEN_WIDTH * 2; x++) 
            *(VGA_BASE + y * VGA_SCREEN_WIDTH * 2 + x)
            = *(VGA_BASE + (y + 1) * VGA_SCREEN_WIDTH * 2 + x);
    for (int pos = (VGA_SCREEN_HEIGHT - 1) * VGA_SCREEN_WIDTH
        ; pos < VGA_SCREEN_WIDTH * VGA_SCREEN_HEIGHT; pos++) {
        *(VGA_BASE + pos * 2) = ' ';
        *(VGA_BASE + pos * 2 + 1) = 0;
...
void put_char2pos(unsigned char c, int color, unsigned short int pos) {
    ...
    //
    if (cursor_y >= VGA_SCREEN_WIDTH*VGA_SCREEN_HEIGHT)
        scroll_screen();
    set_cursor_pos(pos % (VGA_SCREEN_WIDTH * VGA_SCREEN_HEIGHT));
}
