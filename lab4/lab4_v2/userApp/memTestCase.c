...
int mytest(int argc, unsigned char** argv) {
	char* buf1 = (char*)malloc(30);
	

	char* str = { "dengyichuang_PB19000050" };
	...
	*(buf1 + 29) = '\000';
	...
	myPrintf(0x5, "allocated.\n");
	myPrintf(0x5, "BUF1(addr=0x%x) filled with : ", (unsigned long)buf1);
	myPrintf(0x7, buf1);
	myPrintf(0x7, "\n");
...
void memTestCaseInit(void){	
	...
	addNewCmd("mytest\0", mytest, NULL, "MyTest.\0");
}
