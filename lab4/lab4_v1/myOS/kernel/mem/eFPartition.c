...
unsigned long eFPartitionInit(unsigned long start, unsigned long perSize, unsigned long n){
	...
	*/
	...
	myPrintk(0x7, "\npersize%d\n", perSize);
	//place EEB after eFP
	unsigned long n_posi = start + eFPartition_size;
	peFP->firstFree = n_posi;
	struct EEB* pEEB = (struct EEB*)n_posi;
	//split idle block and link EEBs
	for (int i = 0; i < n - 1; i++){
...
unsigned long eFPartitionAlloc(unsigned long EFPHandler){
	...
	*/
	struct eFPartition* peFP = (struct eFPartition*)EFPHandler;
	unsigned long firstsave = peFP->firstFree;
	struct EEB* pEEB = (struct EEB*)firstsave;
	// point move to next idle block 
...
unsigned long eFPartitionFree(unsigned long EFPHandler,unsigned long mbStart){
	...
	*/
	...
	struct eFPartition* peFP = (struct eFPartition*)EFPHandler;
	//get first EEB
	unsigned long point = (EFPHandler + eFPartition_size);
	struct EEB* pEEB = (struct EEB*)point;
	//arrange the list, alter next_start and firstFree 
	while (point <= mbStart){
